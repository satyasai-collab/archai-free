...
- **Трёхролевая схема** - `Executor` (основная модель), `Expert` (судья s чистым
  контекстом и нулём инструментов), `Critic` (жёсткий QA, только чтение).
...
Поток: `Executor` получает задачу от `User`, вызывает `Expert` для декомпозиции и
